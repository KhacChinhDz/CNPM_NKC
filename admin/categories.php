			...
			<form action="" id="manage-category">
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								<label class="control-label">Giá</label>
								<input type="number" class="form-control text-right" name="price" step="any">
							</div>
							<div class="form-group">
								<label for="" class="control-label">Ảnh</label>
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								...
								<button class="btn btn-sm btn-default col-sm-3" type="button" onclick="$('#manage-category').get(0).reset()"> Hủy</button>
							</div>
						</div>
					</div>
				</div>
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										...
										<p>Giá : <b><?php echo number_format($row['price']) . " VNĐ" ?></b></p>
									</td>
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_cat" type="button" data-id="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-price="<?php echo $row['price'] ?>" data-cover_img="<?php echo $row['cover_img'] ?>">Sửa</button>
...
<script>
	...
	$('#manage-category').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Đã thêm thành công",'success')
					setTimeout(function(){
						location.reload()
					},1500)
					...
					alert_toast("Cập nhật thành công",'success')
					setTimeout(function(){
						location.reload()
					},1500)
	...
	function delete_cat($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Xóa thành công!",'success')
					setTimeout(function(){
						location.reload()
					},1500)
